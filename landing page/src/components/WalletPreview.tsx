          ...
          >
            <Card className={`p-8 border-border transition-all duration-500 rounded-3xl ${isFrozen ? 'bg-destructive/5' : 'bg-card'}`}>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                <Button disabled={isFrozen} className="bg-primary hover:bg-primary/90 gap-2 h-12" onClick={() => window.location.href = 'http://localhost:5173/transactions'}>
                  <Send className="w-4 h-4" /> Send
                </Button>
                <Button disabled={isFrozen} variant="outline" className="gap-2 h-12 border-border" onClick={() => window.location.href = 'http://localhost:5173/transactions'}>
